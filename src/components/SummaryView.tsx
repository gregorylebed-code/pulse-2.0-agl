...
export default function SummaryView({ notes, students, classes, lessonHistory, saveLessonHistory, summaries, setSummaries }: Props) {
  ...
  const handleDownloadPDF = (cls: string, topic: SELTopic, lesson: SELLesson) => {
    ...
    y += 8;
    addText('Generated by ShortHand', 8, false, [180, 180, 180]);
      ...
      {hasNoNotes && (
        <div className="text-center py-14 space-y-3 bg-white rounded-[28px] border border-dashed border-slate-200 px-6">
          ...
          <p className="text-xs text-slate-400 leading-relaxed max-w-xs mx-auto">
            Head to the <span className="font-bold text-sage">Notes tab</span> and add a few observations. Once you do, they'll show up here grouped by class — and you can generate AI summaries and SEL lesson suggestions.
          </p>
        </div>
      )}
