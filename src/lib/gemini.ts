...
export async function summarizeNotes(notes: Note[], length: 'Quick Note' | 'Standard' | 'Detailed' = 'Standard', shoutouts: Shoutout[] = [], studentPronouns?: string | null, studentName?: string): Promise<{ report: ReportData | null; pronounInfo: PronounInfo }> {
  ...
  const studentFirstName = studentName?.split(' ')[0] || notes[0]?.student_name?.split(' ')[0] || 'your child';
